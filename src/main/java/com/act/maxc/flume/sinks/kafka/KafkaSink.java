...
/**
 * A Flume Sink that can publish messages to Kafka. This is a general
 * implementation that can be used with any Flume agent and a channel. The
 * message can be any event and the key is a string that we read from the header
 * For use of partitioning, use an interceptor to generate a header with the
 * partition key
 * <p/>
 * Mandatory properties are: brokerList -- can be a partial list, but at least 2
 * are recommended for HA
 * <p/>
 * <p/>
 * however, any property starting with "kafka." will be passed along to the
 * Kafka producer Read the Kafka producer documentation to see which
 * configurations can be used
 * <p/>
 * Optional properties topic - there's a default, and also - this can be in the
 * event header if you need to support events with different topics batchSize -
 * how many messages to process in one batch. Larger batches improve throughput
 * while adding latency. requiredAcks -- 0 (unsafe), 1 (accepted by at least one
 * broker, default), -1 (accepted by all brokers) useFlumeEventFormat -
...
public class KafkaSink extends AbstractSink implements Configurable, BatchSizeSupported {
	...
	private Map<String, List<Event>> bufferedEventListsMap;
	...
	private void translateOldProps(Context ctx) {
		...
		if (!(ctx.containsKey(BOOTSTRAP_SERVERS_CONFIG))) {
			...
			if (brokerList == null || brokerList.isEmpty()) {
				...
				logger.warn("{} is deprecated. Please use the parameter {}", BROKER_LIST_FLUME_KEY,
	...
	/**
	 * 
	 * 将event存在bufferedEventListsMap中，等到waitTime一到则全部发送到kafka
	 * 
	 * @param event
	 */
	private void categorizeBufferEvent(Event event) {
